...
class GeneticSolution(ConfigMixin, GeneticParametersBaseModel):
    ...
    def optimization_solution(self) -> OptimizationSolution:
        ...
        # DVhub 15-min fix (2026-05-29): the genetic core emits total_slots =
        # prediction.hours * slots_per_hour entries (192 at interval=900s). This
        # serializer previously hard-coded hourly indexing (start_day_hour,
        # freq="1h", add(hours=...)), which stretched 48h of 15-min slots across
        # 8 days and reported the wrong resolution. Index by SLOT instead. At
        # interval=3600s this is byte-identical to the old hourly behaviour.
        ...
        # power(W) -> energy per slot(Wh): multiply by slot duration in hours.
    ...
    def energy_management_plan(self) -> EnergyManagementPlan:
        ...
        start_datetime = get_ems().start_datetime
        # DVhub 15-min fix (2026-05-29): index by slot, not hour (see
        # optimization_solution). Instruction execution_times were previously
        # spaced add(hours=...), placing 15-min intent on an hourly grid.
        interval_s = int(self.config.optimization.interval or 3600)
        slots_per_hour = max(1, 3600 // interval_s)
        slot_minutes = max(1, interval_s // 60)
        start_local = start_datetime.in_timezone(self.config.general.timezone)
